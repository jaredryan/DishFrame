...
describe("mealplans service", () => {
  ...
  /**
   * Slice 21 empty-account audit correction: `getRecommendations`'s
   * `totalRecipeCount`/`eligibleRecipeCount` drive which of the three
   * Meal Plan Recommendations empty-state messages the UI shows (no
   * owned Recipes vs. owned-but-ineligible vs. filtered-to-nothing).
   * `countRecipeRecommendationEligibility`'s `where` is deliberately the
   * same clause `listRecommendationCandidates` uses for ranking, not a
   * hand-rewritten copy — these tests prove that in practice by
   * cross-checking the count against `recommendations.length` once every
   * optional filter is enabled, not just by asserting each in isolation.
   */
  describe("recommendation eligibility (§80)", () => {
    it("reports zero/zero when the account owns no Recipes at all", async () => {
      const user = await createTestUser();
      userId = user.id;
      await initializeNewUser(userId);
      // Owning a Part must not count as owning a Recipe.
      await dishService.createDish(
        userId,
        "PART",
        content({ title: "Pickled Onions" }),
      );

      const result = await mealPlanService.getRecommendations(userId);

      expect(result.totalRecipeCount).toBe(0);
      expect(result.eligibleRecipeCount).toBe(0);
      expect(result.recommendations).toEqual([]);
    });

    it("counts owned-but-ineligible Recipes in the total without making them eligible", async () => {
      const user = await createTestUser();
      userId = user.id;
      await initializeNewUser(userId);

      // Archived at creation.
      await dishService.createDish(
        userId,
        "RECIPE",
        content({ title: "Old Stew", stage: "ARCHIVED" }),
      );
      // Archived via the real archive flow (Stage -> ARCHIVED derives
      // archivedAt through the same `updateDishStage`/`nextArchivedAt`
      // path `archiveDish` uses in production).
      const archivedLaterId = await dishService.createDish(
        userId,
        "RECIPE",
        content({ title: "Retired Curry", stage: "ACTIVE" }),
      );
      await dishService.archiveDish(userId, archivedLaterId);
      ...
      expect(result.totalRecipeCount).toBe(2);
      expect(result.eligibleRecipeCount).toBe(0);
      expect(result.recommendations).toEqual([]);
    });

    it("eligible count matches recommendations.length exactly once every optional filter is enabled, and stays fixed while filters narrow the results", async () => {
      const user = await createTestUser();
      userId = user.id;
      await initializeNewUser(userId);
      ...
      await dishService.createDish(
        userId,
        "RECIPE",
        content({ title: "Weeknight Active", stage: "ACTIVE" }),
      );
      await dishService.createDish(
        userId,
        "RECIPE",
        content({ title: "Test Kitchen", stage: "EXPERIMENTAL" }),
      );
      await dishService.createDish(
        userId,
        "RECIPE",
        content({ title: "Someday Idea", stage: "IDEA" }),
      );
      await dishService.createDish(
        userId,
        "RECIPE",
        content({ title: "Retired Bake", stage: "ARCHIVED" }),
      );
      // A Part must not be counted as a Recipe either way.
      await dishService.createDish(
        userId,
        "PART",
        content({ title: "Pickled Onions" }),
      ...
      const defaultFilters = await mealPlanService.getRecommendations(userId);
      // §80.1: Experimental/Idea are excluded by default, but they are
      // still *eligible* (a filter can surface them) — unlike the
      // Archived one, which no filter combination ever restores. Proves
      // eligibleRecipeCount tracks the unconditional rule, not whatever
      // the current filter selection happens to return.
      expect(defaultFilters.totalRecipeCount).toBe(4);
      expect(defaultFilters.eligibleRecipeCount).toBe(3);
      expect(defaultFilters.recommendations.map((r) => r.title)).toEqual([
        "Weeknight Active",
      ]);

      const allFilters = await mealPlanService.getRecommendations(userId, {
        includeExperimental: true,
        includeIdea: true,
      });
      // Same account, same moment — the count must not have changed just
      // because the filters did.
      expect(allFilters.totalRecipeCount).toBe(4);
      expect(allFilters.eligibleRecipeCount).toBe(3);
      // The direct proof the count and the ranking rules cannot drift
      // apart: at maximal filters, the ranked result and the eligible
      // count agree exactly.
      expect(allFilters.recommendations).toHaveLength(
        allFilters.eligibleRecipeCount,
      );
      expect(allFilters.recommendations.map((r) => r.title).sort()).toEqual([
        "Someday Idea",
        "Test Kitchen",
        "Weeknight Active",
      ]);
