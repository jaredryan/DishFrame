 ...
 * shape a given item is. A one-item collection collapses into a `"single"`
 * view using the child's own `DirectShare` id directly (accept/decline/
 * cancel/preview all operate on any `DirectShare` id regardless of whether
 * it belongs to a collection), so it behaves exactly like a legacy single
 * send rather than becoming a visually special one-item group.
 */
