...
- **`Dish.currentStructuralSearchText`** (Section names + linked-Part display names, for the current Dish Version's `PartLink`s) is refreshed by the version-creation transaction, since Section names and *which* Parts are linked are genuinely Version-owned content. **Settled by the owner (was "Open question for Slice 6," now closed — `PRODUCT_SPEC.md` §68.5):** the linked-Part-name component resolves from each `PartLink.targetDishId`'s Part's live `Dish.currentTitle` — **never** from `targetDishVersionId`'s own `title` column, which is an inert historical mirror, not a per-Version name snapshot (title is stable Dish/Part identity, §7.1). `targetDishVersionId` still pins the exact linked *content* (Ingredients/Instructions/etc.), only the displayed name is resolved live. Consequence: version-creation is no longer the *only* mutation path that can change this field — renaming a Part must also refresh `currentStructuralSearchText` on every Recipe/Part whose current Version links to it (a new Slice 6 mutation path, not designed further here).
- **`Dish.currentTitle`** is **not** Version-owned after all (Version-trigger and Slice 5 image correction pass) — title turned out to be stable Recipe/Part identity, not content living on any `DishVersion` row, so it is written directly by every mutation that changes the title (an ordinary metadata-only update, same as `Dish.stage`/`Dish.cuisine`), in addition to being carried forward whenever a Version-creating save also changes it. It is refreshed by more than just the version-creation transaction now.
