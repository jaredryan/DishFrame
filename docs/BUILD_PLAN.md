...
This groups by **domain cohesion and foreign-key dependency order**, not by Tier label, per the owner's explicit direction — see `docs/PRISMA_SCHEMA_PROPOSAL.md` for the full rationale and the literal SQL. All four migrations are applied at once, before any Slice 3 UI work begins; Slice 2 below is updated accordingly.
...
- **Major components:** Plan builder (date range, entry add/remove, planned-meal allocation with over/under-allocation warnings per §77.2), recommendation panel (explainable priority ordering per §80.1–80.2), entry-status tracking wired to Cooking Session start/end, linked grocery-list view.
...
- Installable PWA / offline access / native mobile apps (already flagged in `POST_LAUNCH_TODO.md` §I as dependent on a future cooking-mode specification).
...
3. A manual QA pass across phone/tablet/laptop/wide-desktop breakpoints and both themes, mirroring Milestone 1's own "Final Verification" checklist (§20 of `MILESTONE_1.md`) but now exercising real domain screens instead of placeholders.
...
7. Update `README.md` and, if warranted, `POST_LAUNCH_TODO.md` to reflect the domain functionality now live in production — mirroring how Milestone 2 updated those documents after its own scope.
