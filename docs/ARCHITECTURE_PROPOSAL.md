...
- **Dedicated layout** for the `/cook/[sessionId]` segment: no sidebar, a minimal top bar (session title, elapsed time, "End cooking session"), full viewport devoted to the focused unit. This is a `layout.tsx` inside `(app)/cook/[sessionId]/`, not a variant of the standard shell.
...
|---|---|---|
| Cooking Session content selection | Normalized FK references (`CookingSessionUnit.sourceSectionId`/`sourcePartLinkId`, `CookingSessionChecklistItem.sourceId`) into the immutable `DishVersion` graph | The source is already immutable — copying its content would be pure duplication with no safety benefit. Session-owned data (checkoffs, timers, scale) is genuinely new information and lives in its own normalized rows. |
