...
**Authority order followed:** `PRODUCT_SPEC.md` (canonical) → `BRANDING.md` → `PRODUCT_ROADMAP.md` (context only, superseded on conflict) → Milestone 1/Milestone 2 scaffold history (the original scaffold-era planning docs this proposal describes below are no longer kept in the repository; git history is the archive).
...
| Deployment | Live on Vercel (project `dish-frame`, linked via `.vercel/project.json`), production URL `https://dish-frame.vercel.app`, confirmed working: OAuth, Neon, Resend contact form, security headers, Speed Insights, SEO/metadata (Milestone 2 scaffold work complete) |
...
**`prisma/schema.prisma` is the single authoritative source for the exact, literal schema** — every field, relation, referential action, composite key, and raw-SQL constraint. Where the two ever appear to disagree on a mechanical detail (an exact relation name, a specific `onDelete` action, a field present in one but not the other), `prisma/schema.prisma` governs. This section reflects both Gate 1 review passes (round 1: persistent lineage identity, live/materialized `PartLink` state, self-contained Cooking Session history, normalized grocery contributions, per-lineage preferred units, reference-aware image cleanup, recoverable share tokens; round 2: real foreign-key relations throughout, composite Dish/Version pairing keys, `PartLink` container consistency, deleted-source snapshots on Meal Plan/grocery entries, persisted grocery-sync change-tracking, cross-account `ImageAsset` sharing, grocery-category-memory extraction, generalized nutrition basis, complete search coverage, historical per-Part rating preservation, and additional database invariants) — see the numbered "round 1/round 2/round-3 Correction N" annotations throughout this section (§D) for the correction-by-correction account of what changed and why in each pass.
...
model DishVersion {
  ...
  nutritionSourceProvider String?  // "USDA_FDC" — null once detached (§54.4); Slice 13 correction pass: closed enum, not a free-form string
...
**Version-trigger and Slice 5 image correction pass — a second sanctioned mutable-in-place exception, extended by the Slice 13 metadata-classification correction pass.** `description` and `imageAssetId` are Version-associated but mutable, alongside `versionNote`. Editing either field, on the current Version or any historical one, is an ordinary `UPDATE` to the already-saved row (`applyVersionMetadataUpdate`, `src/lib/dishes/service.ts`) and never creates a new Version. This was a genuine design error in the original Slice 5 implementation — both fields were initially treated as ordinary immutable content requiring a new (automatic minor) Version for any change — corrected once the product rule was settled explicitly: Version association does not imply immutability for fields the product intends to be editable independent of cooking-content evolution. The Slice 13 pass extended this same exception to yield/prep time/cook time/difficulty and the full nutrition shape (see Correction 5, below, for that reversal in full) — `applyVersionMetadataUpdate` now writes all of these fields together, one shared mutable-metadata write path rather than a growing set of separate exceptions.

**Correction 5 — nutrition immutability, no third exception (superseded by the Slice 13 metadata-classification correction pass).** This correction originally classified `nutritionSourceProvider`/`nutritionSourceId`/`calories`/`protein`/`carbs`/`fat`/`moreNutrients` as ordinary immutable `DishVersion` content columns, deliberately declining a second mutable-in-place exception alongside `versionNote`/description/image — any nutrition change went through `createSmallUpdate`/`createNewVersion` like any other content edit, never an in-place `UPDATE`.

The Slice 13 metadata-classification correction pass overturned this. Nutrition — calories/protein/carbs/fat, nutrition basis and basis quantity/unit, More nutrients, and the three source-attribution fields (`nutritionSourceProvider`/`nutritionSourceId`/`nutritionSourceName`) — is Version-*scoped* metadata, governed by the same in-place-mutable rule as description/image/yield/prep time/cook time/difficulty (the extended §F.10 exception, below), not preparation content: none of it, alone, ever justifies a new Version. Selecting an FDC result, editing a value, or detaching from a source may still happen freely while a Version is still being composed in the editor (nothing persisted yet, so there is nothing to mutate). Once a `DishVersion` row has been saved, changing a nutrition value or detaching its source is an ordinary in-place `UPDATE` to that exact row (`applyVersionMetadataUpdate`, `src/lib/dishes/service.ts`) — whether the row is the Dish's current Version or a deliberately selected historical one — never a reason by itself to create a new Version. A material content change (Ingredients/Instructions/Sections-and-ordering/linked Parts) combined with a nutrition change still creates exactly one new Version, through the ordinary material-content flow, carrying the submitted nutrition values; the prior Version's own row is never mutated.
...
model PartLink {
  ...
  // Raw-SQL CHECK constraint added in migration (`prisma/schema.prisma`):
...
model CookingSession {
  ...
  // PARTIAL UNIQUE INDEX in raw migration SQL (`prisma/schema.prisma`), not expressed here as an
...
model ShareLink {
  ...
  revokedAt      DateTime?             // set on owner-initiated revocation AND automatically when the source
                                        ...
                                        // above are relaxed — see `prisma/schema.prisma` for the full
...
| **Editing/detaching imported nutrition** | `DishVersion.nutritionSourceProvider`/`nutritionSourceId`/`nutritionSourceName` (nullable) record provenance. Per Correction 5 (§D.2, superseded by the Slice 13 metadata-classification correction pass), this **is** a mutable-in-place field, alongside every other nutrition column and description/image/yield/prep/cook/difficulty: "detach" against an already-saved Version — current or historical — clears all three attribution fields together via `applyVersionMetadataUpdate`, an ordinary in-place `UPDATE` to that exact row, preserving its numeric values and basis unchanged. No new Version is created by a detach alone; combined with a material content change, the new Version carries the submitted (detached or attached) attribution instead, and the prior Version is untouched. |
...
## Q. Final Implementation Updates (Current-State Notes)

Most of this proposal's architecture landed exactly as designed and has been kept current in place throughout (see the numbered `Correction` notes woven through §D–§O, up through Correction 11). This section records the remaining architectural deltas from later slices that are not yet reflected inline above, now that the historical slice/gate reports that originally recorded them have been consolidated and removed.

- **Public marketing pages are fully static** (`/`, `/about`, `/contact`, `/privacy`, `/terms`) — no session-dependent rendering anywhere in that route group. `PublicHeader`, `HeroShowcase`, and `ClosingCta` accept no `signedIn` prop; the marketing layout and the `(share)` layout make no `getServerSession()` call. All five routes prerender (`○` in `next build`) and serve `Cache-Control: s-maxage=31536000`. This is the final state — an earlier, session-aware `signedIn`-prop design (built during the Slice 21 public-site redesign) was superseded by this fully-static approach. Any future public-page component must not reintroduce session-dependent rendering into this route group without a deliberate decision, since it disables static prerendering, CDN caching, and back/forward-cache eligibility.
- **Legal pages** `/privacy` and `/terms` exist (shared `LegalLayout`), reachable from the public footer and listed in `robots.ts`/`sitemap.ts`. Content is grounded in actual implementation (Google-only sign-in, private-by-default data model, Vercel Blob images, Resend contact form not persisted to the database, no analytics/tracking SDK). Both still need professional legal review before broad commercial launch — tracked in `docs/TODO.md`.
