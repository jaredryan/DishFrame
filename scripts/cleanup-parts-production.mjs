#!/usr/bin/env node
// Guarded, rare-use entry point for running `pnpm cleanup:parts:local`
...
//
// Requires CONFIRM_PRODUCTION_DATABASE=yes like cleanup-recipes-production.mjs
// because this can run destructively (--delete).
//
// Deliberately narrow: this only ever invokes cleanup:parts:local against
...
const COMMAND_LABEL = "cleanup:parts:production";
...
const child = spawn(
  "pnpm",
  ["run", "cleanup:parts:local", ...process.argv.slice(2)],
  {
    stdio: "inherit",
